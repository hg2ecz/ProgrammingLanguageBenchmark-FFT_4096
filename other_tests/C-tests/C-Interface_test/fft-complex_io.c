...
void fft_complex_io(int log2point, complex float *xy_out_if, const complex float *xy_in) {
	...
	brev >>= 32-log2point;
	xy_out.re[brev] = creal(xy_in[i]);
	xy_out.im[brev] = cimag(xy_in[i]);

	unsigned int brev2 = brev | (1<<(log2point-1));
	xy_out.re[brev2] = creal(xy_in[i+1]);
	xy_out.im[brev2] = cimag(xy_in[i+1]);
    }
	...
	xy_out_if[i] = xy_out.re[i] + I*xy_out.im[i];
    }
}
