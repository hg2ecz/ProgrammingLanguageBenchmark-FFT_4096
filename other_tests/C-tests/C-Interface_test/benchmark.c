...
int main(int argc, char **argv) {
	...
	xy.re[i]= 1.+i/1000.;
	xy.im[i]= i/2000.;
	cplxin[i] = xy.re[i] + I*xy.im[i];
    }

// FFT-vector io
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gstart);
    for (int i=0; i<FFT_REPEAT; i++) xy_out = fft_vector_io(LOG2FFTSIZE, xy);
...
// FFT-complex io
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gstart);
    for (int i=0; i<FFT_REPEAT; i++) fft_complex_io(LOG2FFTSIZE, cplxout, cplxin);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gend);
