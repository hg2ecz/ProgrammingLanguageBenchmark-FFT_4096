...
static _Complex _Float16 phasevec[32];
...
//	double complex wphase_XY = cos(theta) + sin(theta)*I;
	_Complex _Float16 wphase_XY = phasevec[l2pt++];

	_Complex _Float16 w_XY = 1.0 + 0.0*I;
	for (int m=0; m < mmax; m++) {
	    for (int i=m; i < n; i += istep) {
		_Complex _Float16 tempXY = w_XY *xy_out[i+mmax];
