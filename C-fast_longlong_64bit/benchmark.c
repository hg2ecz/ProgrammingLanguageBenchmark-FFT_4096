...
void printres(int i, struct _sample *d) {
    printf("%3d %16.9f %16.9f\n", i, (double)d[i].i/(1ULL<<INTMULSAMPLE), (double)d[i].q/(1ULL<<INTMULSAMPLE));
}
...
int main() {
    ...
    for(i=0; i<SIZE/2; i++) { xy[i].i = 1*(1ULL<<INTMULSAMPLE); xy[i].q = 0; }
    for(   ; i<SIZE  ; i++) { xy[i].i =-1*(1ULL<<INTMULSAMPLE); xy[i].q = 0; }
