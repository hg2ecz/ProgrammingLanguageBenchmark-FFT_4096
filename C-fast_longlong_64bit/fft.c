...
void fft(int log2point, struct _sample *restrict xy_out, const struct _sample *restrict xy_in) {
	...
	for (i=0; i<32; i++) {
	    int point = 2<<i;
	    phasevec[i].i = (1ULL<<INTMUL)*cos(-2*M_PI/point);
	    phasevec[i].q = (1ULL<<INTMUL)*sin(-2*M_PI/point);
	}
	phasevec_exist = 1;
    }
...
//	double complex wphase_XY = cos(theta) + sin(theta)*I;
	...
	struct _sample w_XY = { 1ULL<<INTMUL, 0 };
	for (int m=0; m < mmax; m++) {
	    for (int i=m; i < n; i += istep) {
		struct _sample tempXY;
		tempXY.i = ((__int128)w_XY.i * xy_out[i+mmax].i - (__int128)w_XY.q * xy_out[i+mmax].q) >> INTMUL;
		tempXY.q = ((__int128)w_XY.i * xy_out[i+mmax].q + (__int128)w_XY.q * xy_out[i+mmax].i) >> INTMUL;
		xy_out[i+mmax].i  = xy_out[i].i - tempXY.i;
		xy_out[i+mmax].q  = xy_out[i].q - tempXY.q;
		xy_out[i     ].i += tempXY.i;
		xy_out[i     ].q += tempXY.q;
	    }
	    long w_tmp = ((__int128)w_XY.i * wphase_XY.i - (__int128)w_XY.q * wphase_XY.q) >> INTMUL;
	    w_XY.q = ((__int128)w_XY.i * wphase_XY.q + (__int128)w_XY.q * wphase_XY.i) >> INTMUL;
	    w_XY.i = w_tmp;
	}
	mmax=istep;
    }
