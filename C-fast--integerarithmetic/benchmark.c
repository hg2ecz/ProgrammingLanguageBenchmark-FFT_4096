...
void printres(int i, struct _sample *d) {
    printf("%3d %16.3f %16.3f\n", i, (double)d->fract[i][0]/(1<<INTMULSAMPLE), (double)d->fract[i][1]/(1<<INTMULSAMPLE));
}

#define SIZE (1<<LOG2FFTSIZE)

struct _sample xy;
struct _sample xy_out_fft;
...
int main() {
    ...
    for(i=0; i<SIZE/2; i++) { xy.fract[i][0] = 1*(1<<INTMULSAMPLE); xy.fract[i][1] = 0; }
    for(   ; i<SIZE  ; i++) { xy.fract[i][0] =-1*(1<<INTMULSAMPLE); xy.fract[i][1] = 0; }
