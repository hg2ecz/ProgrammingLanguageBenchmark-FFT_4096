...
void fft_estimate(float complex *xy_out, float complex *xy) {
    double eltime;
    ...
    printf("FFTW_ESTIMATE: %6d piece(s) of %d pt FFT;  %9.5f ms/piece\n", FFT_REPEAT, 1<<LOG2FFTSIZE, eltime/FFT_REPEAT);
}

void fft_measure(float complex *xy_out, float complex *xy) {
    double eltime;
    struct timespec gstart, gend;

    fftwf_plan plan_forward;
    plan_forward = fftwf_plan_dft_1d(SIZE, xy, xy_out, FFTW_FORWARD, FFTW_MEASURE );
