...
void fft_cplx(int log2point, complex float *xy_out_if, const complex float *xy_in) {
    FLOAT_TYPE out_re[1<<log2point] __attribute__ ((aligned (32)));
    FLOAT_TYPE out_im[1<<log2point] __attribute__ ((aligned (32)));
	...
	out_re[i] = creal(xy_in[brev]);
	out_im[i] = cimag(xy_in[brev]);

	unsigned int brev1 = brev + (2<<(log2point-2)); // 01 --> 10
	out_re[i+1] = creal(xy_in[brev1]);
	out_im[i+1] = cimag(xy_in[brev1]);

	unsigned int brev2 = brev + (1<<(log2point-2)); // 10 --> 01
	out_re[i+2] = creal(xy_in[brev2]);
	out_im[i+2] = cimag(xy_in[brev2]);

	unsigned int brev3 = brev + (3<<(log2point-2)); // 11 --> 11
	out_re[i+3] = creal(xy_in[brev3]);
	out_im[i+3] = cimag(xy_in[brev3]);
    }
	...
	xy_out_if[i] = out_re[i] + I*out_im[i];
    }
}
