...
//	double complex wphase_XY = cos(theta) + sin(theta)*I;
	...
	for (int m=0; m < mmax; m++) {
		...
		tempXY.i = sub(mul(w_XY.i, xy_out[i+mmax].i), mul(w_XY.q, xy_out[i+mmax].q));
		tempXY.q = add(mul(w_XY.i, xy_out[i+mmax].q), mul(w_XY.q, xy_out[i+mmax].i));
		xy_out[i+mmax].i = sub(xy_out[i].i, tempXY.i);
		xy_out[i+mmax].q = sub(xy_out[i].q, tempXY.q);
		xy_out[i     ].i = add(xy_out[i     ].i, tempXY.i);
		xy_out[i     ].q = add(xy_out[i     ].q, tempXY.q);
	    }
	    float w_tmp = sub(mul(w_XY.i, wphase_XY.i), mul(w_XY.q, wphase_XY.q));
	    w_XY.q      = add(mul(w_XY.i, wphase_XY.q), mul(w_XY.q, wphase_XY.i));
	    w_XY.i      = w_tmp;
	}
	mmax=istep;
    }
